...
- (void)viewDidLoad {
    ...
    [stack addArrangedSubview:self.planetButton];
    [stack addArrangedSubview:self.headButton];
    [stack addArrangedSubview:self.treeButton];
    [stack addArrangedSubview:self.landscapeButton];
...
-(DrawButton*) headButton {
    if (_headButton){
        ...
        DrawButton *button = [[DrawButton alloc] init];
        [button setTitle:@"Head" forState:UIControlStateNormal];
        [button addTarget:self action:@selector(handlerButton:) forControlEvents:UIControlEventTouchUpInside];
        _headButton = button;
        return button;
    }
...
-(DrawButton*) treeButton {
    if (_treeButton){
        ...
        DrawButton *button = [[DrawButton alloc] init];
        [button setTitle:@"Tree" forState:UIControlStateNormal];
        [button addTarget:self action:@selector(handlerButton:) forControlEvents:UIControlEventTouchUpInside];
        _treeButton = button;
        return button;
    }
...
-(DrawButton*) landscapeButton {
    if (_landscapeButton){
        ...
        DrawButton *button = [[DrawButton alloc] init];
        [button setTitle:@"Landscape" forState:UIControlStateNormal];
        [button addTarget:self action:@selector(handlerButton:) forControlEvents:UIControlEventTouchUpInside];
        _landscapeButton = button;
        return button;
    }
}
